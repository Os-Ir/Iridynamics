...
public class BonfireBlockEntity extends SyncedBlockEntity implements ITickable, IIgnitable, IBlockEntityHolder<BonfireBlockEntity> {
    ...
    public void tick(Level level, BlockPos pos, BlockState state) {
        if (!level.isClientSide) {
            ...
            this.inventory.getStackInSlot(1).getCapability(HeatCapability.HEAT).ifPresent((item) -> HeatUtil.heatExchange(this.heat, item, INVENTORY_RESISTANCE));
            this.inventory.getStackInSlot(2).getCapability(HeatCapability.HEAT).ifPresent((item) -> HeatUtil.heatExchange(this.heat, item, INVENTORY_RESISTANCE));
            if (this.ignite) {
                double maxConsume;
